...
const CPU_SPEED = 15.0;
const CPU_REACT = 1.0;
const CPU_ERROR_Y = 0;
const CPU_MISTAKE_CHANCE = 0;
const CPU_MISTAKE_DUR = 0;
...
function goalScored(who) {
	...
	setTimeout(() => {
        if (score.p >= MAX_SCORE || score.cpu >= MAX_SCORE) {
            ...
            if (playerWon) {
                // Écran d'attente pendant la requête
                goWho.textContent = "ANALYSE...";
                goWho.style.color = "#00d4ff";
                goWho.style.textShadow = "0 0 30px #00d4ff, 0 0 60px rgba(0,212,255,0.4)";
                goWinsEl.textContent = "Connexion au serveur...";
                ...
                fetch('/api/victoire', {
                    ...
                    body: JSON.stringify({ score_p: score.p })
                })
                .then(response => response.json())
                .then(data => {
                    // Le serveur a répondu, on affiche le flag !
                    goWho.textContent = "FLAG OBTENU";
                    goWinsEl.textContent = data.flag || "Erreur : Flag manquant";
                    document.getElementById("go-face").textContent = "😎";
                    
                    // On lance les confettis et la musique de victoire
                    playSound("victory");
                    spawnConfetti();
                ...
                .catch(err => {
                    goWho.textContent = "ERREUR RÉSEAU";
                    goWinsEl.textContent = "Impossible de joindre le serveur.";
                    document.getElementById("go-face").textContent = "⚠️";
                });

            } else {
                // Défaite contre l'IA
                goWho.textContent = "CPU WINS";
                goWho.style.color = "#ff2d55";
                goWho.style.textShadow = "0 0 30px #ff2d55, 0 0 60px rgba(255,45,85,0.4)";
                goWinsEl.textContent = "L'IA EST TROP FORTE... CHERCHE DANS LE CODE !";
                document.getElementById("go-face").textContent = "🤖";
            }

            // Affichage du score final et fin de l'animation
